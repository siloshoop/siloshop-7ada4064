ALTER TABLE public.platform_payment_settings
  ADD COLUMN IF NOT EXISTS electronic_payment_enabled boolean NOT NULL DEFAULT false,
  ADD COLUMN IF NOT EXISTS electronic_payment_instructions text NOT NULL DEFAULT '',
  ...
  ADD COLUMN IF NOT EXISTS apply_to_all_products boolean NOT NULL DEFAULT false;

ALTER TABLE public.platform_payment_settings
  DROP CONSTRAINT IF EXISTS platform_shipping_fee_non_negative;
ALTER TABLE public.platform_payment_settings
  ADD CONSTRAINT platform_shipping_fee_non_negative CHECK (shipping_fee >= 0 AND shipping_fee <= 100000000);

ALTER TABLE public.orders DROP CONSTRAINT IF EXISTS orders_payment_method_check;
ALTER TABLE public.orders ADD CONSTRAINT orders_payment_method_check
  CHECK (payment_method = ANY (ARRAY['cod','sham_cash','electronic']));
...
ALTER TABLE public.payments ADD CONSTRAINT payments_method_valid
  CHECK (payment_method = ANY (ARRAY['cod','cash','sham_cash','electronic','syriatel','mtn','card','bank_transfer']));

DROP FUNCTION IF EXISTS public.get_platform_payment_options();
...
AS $function$
  SELECT
    ...
    s.electronic_payment_enabled AND s.is_active,
    s.free_shipping,
    s.shipping_fee,
    s.apply_to_all_products,
    s.instructions,
    s.electronic_payment_instructions
  FROM public.platform_payment_settings s
  WHERE s.id = 1;
$function$;

REVOKE EXECUTE ON FUNCTION public.get_platform_payment_options() FROM anon;
GRANT EXECUTE ON FUNCTION public.get_platform_payment_options() TO authenticated, service_role;
...
BEGIN
  ...
  -- Platform (Turkish) products: shipping + payment methods come from the
  -- platform settings. Seller products keep per-product shipping and COD only,
  -- unless the admin chose to apply the platform settings to all products.
